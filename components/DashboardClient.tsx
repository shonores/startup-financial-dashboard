...
export default function DashboardClient({ data, onUploadMore }: Props) {
  const [windowMonths, setWindowMonths] = useState(6);
  const [burnViewMode, setBurnViewMode] = useState<BurnViewMode>("all");

  const { monthlySummaries: summaries, vendors, meta } = data;

  const metrics = computeRunwayMetrics(summaries, vendors, windowMonths);
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            {meta.filesProcessed} account{meta.filesProcessed > 1 ? "s" : ""} merged
            · {meta.monthsCovered}
            · {meta.processedRows.toLocaleString()} transactions
          </p>
          {meta.parseErrors?.map((e) => (
            <p key={e} className="text-yellow-500 text-xs mt-1">⚠ {e}</p>
          ))}
        </div>
        <button onClick={onUploadMore} className="btn-ghost text-sm">
          Upload More
        </button>
      </div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className="text-xs text-slate-600">— used for avg burn &amp; projections</span>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        ...
        <KPICard
          ...
          valueClassName={metrics.runwayMonths < 6 ? "text-red-400" : "text-slate-100"}
        />
        <KPICard
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          <h2 className="text-sm font-semibold text-slate-300 mb-4">Cash Trajectory</h2>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-slate-300">Monthly Financials</h2>
            <div className="flex gap-1">
              {BURN_VIEWS.map((v) => (
                <button
                  key={v.mode}
